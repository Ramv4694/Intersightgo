...
**FailoverEnabled** | Pointer to **bool** | Enabling failover ensures that on an uplink failure, the traffic automatically fails over from one uplink to another. Failover applies only to Cisco VIC adapters that are connected to a Fabric Interconnect cluster. The uplink, determines the primary uplink in case of a failover. | [optional] [default to false]
